...
string keying(string a){
    MD5 test;
    return test.encode(a);
}
#undef A
#undef B
#undef C
...
void bigth(int a,int b){
	char SB_0[64];
	sprintf(SB_0,"mode con cols=%d lines=%d",a,b);
	system(SB_0);
	return ;
}

string uuid(){
	system("(wmic csproduct get uuid)>ls.txt");
	ifstream bg("ls.txt");
	string d,e;
	getline(bg,d);
	getline(bg,e);
...
string time(){
	...
	time(&SB_0);
	char SB_1[64];
	strftime(SB_1, sizeof(SB_1), "%Y-%m-%d %H:%M:%S", localtime(&SB_0));
	return SB_1;
}
/*
string login(){
	map<string,string>lck;
	string e,f;
	while(in>>e>>f)
		lck[e]=f;
	system("cls");
	...
	while(1){
		...
		if(b==13)break;
			...
			if(a.size()>0){
				a.erase(a.end() - 1);
				system("cls");
				cout<<"Name:"<<c<<endl<<"Key:";
				for(int i=1;i<=a.size();i++)cout<<'%';
			}
		}else a+=b,cout<<'%';
	}
	if(lck.count(c)&&lck[c]==a){
		Sleep(1000);
		system("cls");
